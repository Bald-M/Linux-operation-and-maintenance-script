...
class Configure_FTP():
    ...
    def global_config(self):
        ...
        # 全局配置项
        write_enable = os.popen(r"sed -n '/^write_enable/p' %s"%local_ftp_file)
        write_enable_info = write_enable.read().strip()
        download_enable = os.popen(r"sed -n '/download_enable/p' %s "%local_ftp_file)
        download_enable_info = download_enable.read().strip()
        userlist_enable = os.popen(r"sed -n '/userlist_enable/p' %s "%local_ftp_file)
        userlist_enable_info = userlist_enable.read().strip()
        userlist_deny = os.popen(r"sed -n '/userlist_deny/p' %s"%local_ftp_file)
        userlist_deny_info = userlist_deny.read().strip()
        max_clients = os.popen(r"sed -n '/max_clients/p' %s " % local_ftp_file)
        max_clients_info = max_clients.read().strip()
        max_per_ip = os.popen(r"sed -n '/max_per_ip/p' %s "%local_ftp_file)
        max_per_ip_info = max_per_ip.read().strip()
        listen = os.popen(r"sed -n '/^listen=/p' %s "%local_ftp_file)
        listen_info = listen.read().strip()
        listen_address = os.popen(r"sed -n '/listen_address/p' %s "%local_ftp_file)
        listen_address_info = listen_address.read().strip()
        listen_port = os.popen(r"sed -n '/listen_port/p' %s "%local_ftp_file)
        ...
        if enable_write.lower() == "y":
            os.system(r"sed -i 's/%s/write_enable=YES/' %s" % (write_enable_info, local_ftp_file))
        # 拒绝写入
        elif enable_write.lower() == "n":
            os.system(r"sed -i 's/%s/write_enable=NO/' %s" % (write_enable_info, local_ftp_file))
        # 拒绝写入
        else:
            os.system(r"sed -i 's/%s/write_enable=NO/' %s" % (write_enable_info, local_ftp_file))
        ...
        if enable_download.lower() == "y":
            os.system(r"sed -i 's/%s/download_enable=YES/' %s"%(download_enable_info,local_ftp_file))
        # 拒绝下载
        elif enable_download.lower() == "n":
            os.system(r"sed -i 's/%s/download_enable=NO/' %s"%(download_enable_info,local_ftp_file))
        # 拒绝下载
        else:
            os.system(r"sed -i 's/%s/download_enable=NO/' %s" % (download_enable_info, local_ftp_file))
        ...
        if enable_userlist.lower() == "y":
            os.system(r"sed -i 's/%s/userlist_enable=YES/' %s"%(userlist_enable_info,local_ftp_file))
        # 不启用user_list
        elif enable_userlist.lower() == "n":
            os.system(r"sed -i 's/%s/userlist_enable=NO/' %s"%(userlist_enable_info,local_ftp_file))
        # 不启用user_list
        else:
            os.system(r"sed -i 's/%s/userlist_enable=NO/' %s" % (userlist_enable_info, local_ftp_file))
        ...
        if deny_userlist.lower() == "y":
            os.system(r"sed -i 's/%s/userlist_deny=YES/' %s"%(userlist_deny_info,local_ftp_file))
        # 设置user_list为白名单
        elif deny_userlist.lower() == "n":
            os.system(r"sed -i 's/%s/userlist_deny=NO/' %s"%(userlist_deny_info,local_ftp_file))
        # 设置user_list为白名单
        else:
            os.system(r"sed -i 's/%s/userlist_deny=NO/' %s" % (userlist_deny_info, local_ftp_file))
        ...
        if max_connection.isdigit():
            os.system(r"sed -i 's/%s/max_clients=%s/' %s" % (max_clients_info, max_connection, local_ftp_file))
        else:
            os.system(r"sed -i 's/%s/max_clients=0/' %s"%(max_clients_info,local_ftp_file))

        # 配置最多文件下载数量
        max_per_download = input("最多允许每个用户下载多少个文件（回车默认不做限制）：")
        if max_per_download.isdigit():
            os.system(r"sed -i 's/%s/max_per_ip=%s/' %s" % (max_per_ip_info, max_per_download, local_ftp_file))
        else:
            os.system(r"sed -i 's/%s/max_per_ip=0/' %s" % (max_per_ip_info,local_ftp_file))


        listening = input("是否开启监听地址(y/n/回车默认不开启)：")
        # 开启侦听地址
        if listening.lower() == "y":
            os.system(r"sed -i 's/%s/listen=YES/' %s"%(listen_info,local_ftp_file))
            # 配置侦听地址
            listening_address = input("请输入监听FTP服务的IP地址（回车默认全网段）：")
            if listening_address == "":
                os.system(r"sed -i 's/%s/listen_address=0.0.0.0/' %s" % (listen_address_info, local_ftp_file))
            else:
                os.system(
                    r"sed -i 's/%s/listen_address=%s/' %s" % (listen_address_info, listening_address, local_ftp_file))

            listening_port = input("请输入监听FTP服务的端口号（回车端口号默认21号）：")
            if listening_port == "":
                os.system(r"sed -i 's/%s/listen_port=21/' %s" % (listen_port_info, local_ftp_file))
            else:
                os.system(r"sed -i 's/%s/listen_port=%s/' %s" % (listen_port_info, listening_port, local_ftp_file))
        # 不开启监听地址
        elif listening.lower() == "n":
            os.system(r"sed -i 's/%s/listen=NO/' %s"%(listen_info,local_ftp_file))
        # 不开启监听地址
        else:
            os.system(r"sed -i 's/%s/listen=NO/' %s" % (listen_info, local_ftp_file))
    ...
    def config_local_users(self):
        ...
        # 本地用户配置项
        local_enable = os.popen("sed -n '/local_enable/p' %s"%local_ftp_file)
        local_enable_info = local_enable.read().strip()
        local_umask = os.popen("sed -n '/local_umask/p' %s"%local_ftp_file)
        local_umask_info = local_umask.read().strip()
        local_root = os.popen("sed -n '/local_root/p' %s"%local_ftp_file)
        local_root_info = local_root.read().strip()
        local_max_rate = os.popen("sed -n '/local_max_rate/p' %s"%local_ftp_file)
        ...
        if enable_local.lower() == "y":
            os.system("sed -i 's/%s/local_enable=YES/' %s"%(local_enable_info,local_ftp_file))
            os.system("sed -i 's/%s/local_umask=022/' %s"%(local_umask_info,local_ftp_file))
            os.system("sed -i 's/%s/local_max_rate=0/' %s"%(local_max_rate_info,local_ftp_file))

            local_root_path = input("请输入本地用户的根目录（回车默认设置为/var/ftp）：")
            if local_root_path == "":
                os.system("sed -i 's/%s/local_root=\/var\/ftp/' %s" % (local_root_info.replace("/", "\/"), local_ftp_file))
            else:
                os.system("sed -i 's/%s/local_root=%s/' %s" % (
                local_root_info.replace("/", "\/"), local_root_path.replace("/", "\/"), local_ftp_file))
        # 拒绝本地用户
        elif enable_local.lower() == "n":
            os.system("sed -i 's/%s/local_enable=NO/' %s"%(local_enable_info,local_ftp_file))
            os.system("sed -i 's/%s/#local_umask=022/' %s" % (local_umask_info, local_ftp_file))
            os.system("sed -i '/local_root/s/%s/#%s/g' %s"%(local_root_info.replace("/","\/"), local_root_info.replace("/","\/"), local_ftp_file))
            os.system("sed -i 's/%s/#local_max_rate=0/' %s" % (local_max_rate_info, local_ftp_file))
        # 拒绝本地用户
        else:
            os.system("sed -i 's/%s/local_enable=NO/' %s" % (local_enable_info, local_ftp_file))
            ...
            local_root_info.replace("/", "\/"), local_root_info.replace("/", "\/"), local_ftp_file))
            os.system("sed -i 's/%s/#local_max_rate=0/' %s" % (local_max_rate_info, local_ftp_file))
